...
// runOpenAIKeyCommand handles `saker openai-key <action>` subcommands. It is
// the operator-side counterpart to the OpenAI gateway: it manipulates the
// Bearer keys that authenticate /v1/* requests.
//
// Actions:
...
func resolveStore(dataDir string) (*project.Store, error) {
	if dataDir == "" {
	...
	store, err := project.Open(project.Config{
...
func resolveUser(ctx context.Context, store *project.Store, username string) (*project.User, error) {
	...
	row, err := store.LookupUserByUsername(ctx, username, project.UserSourceLocalhost)
	if err == nil && row != nil {
		return row, nil
	}
