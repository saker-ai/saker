...
- The SDK automatically loads MCP servers from `.saker/settings.json` when `ProjectRoot` is set; no manual wiring is needed.
- To disable MCP for this example, run `go run ./examples/02-cli --enable-mcp=false`.
- To add servers, edit `.saker/settings.json` under `mcp.servers`; the SDK handles spec conversion and registration.

## Tips

- Type `exit` to quit
- Only assistant replies are printed
- MCP servers are loaded from `.saker/settings.json` in the project root
- Use `--project-root` to point at a different `.saker` directory if needed
