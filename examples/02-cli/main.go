...
func main() {
	...
	enableMCP := flag.Bool("enable-mcp", true, "enable MCP servers from .saker/settings.json (auto-loaded)")
	flag.Parse()
	...
	if *enableMCP {
		fmt.Println("MCP auto-load enabled; SDK will read .saker/settings.json. Use --enable-mcp=false to disable.")
	}
	fmt.Println()
