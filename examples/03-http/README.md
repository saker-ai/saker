...
```
Defaults to `:8080`. Override with `SAKER_HTTP_ADDR`. Choose a model with `SAKER_MODEL` (default `claude-3-5-sonnet-20241022`). Optionally set `ANTHROPIC_BASE_URL` for custom endpoints.
...
```json
{
  "prompt": "Summarize saker in one sentence",
