...
- **Optional** variables you can adjust:
  ...
  - `SAKER_MODEL` - Override default model (default: `claude-sonnet-4-5-20250929`)
  - `SAKER_HTTP_ADDR` - HTTP server address for example 03 (default: `:8080`)
...
- What each example reads:
  - `examples/01-basic`: `ANTHROPIC_API_KEY` (or `ANTHROPIC_AUTH_TOKEN`), optional `SAKER_MODEL`, optional `ANTHROPIC_BASE_URL`.
  - `examples/02-cli`: same as 01-basic.
  - `examples/03-http`: `ANTHROPIC_API_KEY` (or `ANTHROPIC_AUTH_TOKEN`), `SAKER_HTTP_ADDR` (default :8080), optional `SAKER_MODEL`, optional `ANTHROPIC_BASE_URL`.
