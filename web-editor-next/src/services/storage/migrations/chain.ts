...
// 28 migration classes pull in their transformers (~5k LOC of legacy data
// rewriting). Importing them eagerly would inflate the editor entry chunk
// for every visitor, even though new projects are created at the current
// version (see project-manager.ts) and so never trigger a migration.
//
// This file is dynamic-imported by runStorageMigrations only when a probe
// finds at least one stored project below CURRENT_PROJECT_VERSION.
