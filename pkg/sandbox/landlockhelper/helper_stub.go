...
// On non-Linux platforms Landlock is unavailable so the command runs unsandboxed.
...
// Invoke executes helper mode via self-exec when possible.
...
// ExecuteRequest runs the command without Landlock (unavailable on this platform).
