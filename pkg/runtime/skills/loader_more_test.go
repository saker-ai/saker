...
func TestParseSkillFileMismatchAndInvalid(t *testing.T) {
	...
	// Name mismatch is now a warning, not an error — skill loads using its frontmatter name.
	sf, err := parseSkillFile(path, "demo", config.NewFS(root, nil))
	if err != nil {
		t.Fatalf("expected no error for name mismatch (warning only), got %v", err)
	}
	if sf.Name != "wrong" {
		t.Fatalf("expected skill name %q from frontmatter, got %q", "wrong", sf.Name)
	}
