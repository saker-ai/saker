...
type ActivationContext struct {
	...
	FilePaths      []string // files touched in current context, used for conditional activation
	AvailableTools []string // currently registered tool names, for conditional activation
}
...
func (c ActivationContext) Clone() ActivationContext {
	...
	if len(c.FilePaths) > 0 {
		cloned.FilePaths = append([]string(nil), c.FilePaths...)
	}
	if len(c.AvailableTools) > 0 {
		cloned.AvailableTools = append([]string(nil), c.AvailableTools...)
	}
	return cloned
}
