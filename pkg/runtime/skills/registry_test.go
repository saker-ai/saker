...
func TestActivationDefinition(t *testing.T) {
	...
	match := r.Match(ActivationContext{Prompt: "run inspector now"})
	if len(match) != 1 {
...
func TestRegistryDefaultsAndValidation(t *testing.T) {
	...
	// Matcherless skills should NOT match when the prompt doesn't mention the skill name.
	matches := r.Match(ActivationContext{})
	if len(matches) != 0 {
		t.Fatalf("expected no match for matcherless skill with empty prompt, got %+v", matches)
	}
	// But they should match when the prompt mentions the skill name.
	matches = r.Match(ActivationContext{Prompt: "please run auto-skill"})
	if len(matches) != 1 || matches[0].Skill.definition.Name != "auto-skill" {
		t.Fatalf("expected match when prompt mentions skill name, got %+v", matches)
	}
