...
func TestHandlerLazyLoadsOnFirstExecute(t *testing.T) {
	...
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "lazy")
...
func TestHandlerCachesLoadResult(t *testing.T) {
	isolateAgentsSkillsHome(t)
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "cache")
...
func TestHandlerConcurrentExecuteSingleLoad(t *testing.T) {
	isolateAgentsSkillsHome(t)
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "concurrent")
...
func TestHandlerHotReloadOnFileChange(t *testing.T) {
	isolateAgentsSkillsHome(t)
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "hotreload")
...
func TestHandlerBodyLengthProbe(t *testing.T) {
	isolateAgentsSkillsHome(t)
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "probe")
	body := "probe body"
...
func TestHandlerStatError(t *testing.T) {
	isolateAgentsSkillsHome(t)
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "staterr")
	skillPath := filepath.Join(dir, "SKILL.md")
