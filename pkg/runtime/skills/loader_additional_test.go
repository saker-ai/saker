...
func TestSetSkillFileOpsForTest(t *testing.T) {
	...
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "testops")
	skillPath := filepath.Join(dir, "SKILL.md")
...
func TestLoadFromFSAddsCanonicalSkillMetadata(t *testing.T) {
	isolateAgentsSkillsHome(t)
	root := t.TempDir()
	skillPath := filepath.Join(root, ".saker", "skills", "alpha", "SKILL.md")
...
func TestLoadFromFSWithConfigRootAlsoIncludesAgentsSkills(t *testing.T) {
	root := t.TempDir()
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeSkill(t, filepath.Join(root, "config", "skills", "delta", "SKILL.md"), "delta", "d")
	...
	if len(regs) != 2 {
		t.Fatalf("expected 2 skills (delta + omega), got %d", len(regs))
	}
	names := map[string]bool{}
	for _, r := range regs {
		names[r.Definition.Name] = true
	}
	if !names["delta"] || !names["omega"] {
		t.Fatalf("expected delta and omega, got %+v", regs)
	}
}

func TestLoadFromFSExplicitDirectoriesAlsoIncludeAgentsSkills(t *testing.T) {
	root := t.TempDir()
	home := t.TempDir()
	...
	if len(regs) != 2 {
		t.Fatalf("expected 2 skills (omega + zeta), got %d: %+v", len(regs), regs)
	}
	names := map[string]bool{}
	for _, r := range regs {
		names[r.Definition.Name] = true
	}
	if !names["omega"] || !names["zeta"] {
		t.Fatalf("expected omega and zeta, got %+v", regs)
	}
}
...
func TestHandlerReloadAfterError(t *testing.T) {
	isolateAgentsSkillsHome(t)
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "skills", "reloaderr")
	skillPath := filepath.Join(dir, "SKILL.md")
