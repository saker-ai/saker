...
func TestLoadCommandDirNotDirectory(t *testing.T) {
	root := t.TempDir()
	commandsPath := filepath.Join(root, ".saker", "commands")
...
func TestLazyCommandBodyStatError(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, ".saker", "commands", "staterr.md")
	mustWrite(t, path, "ok")
...
func TestLazyCommandBodyParseErrorAfterReload(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, ".saker", "commands", "reload.md")
...
func TestLoadCommandDirDuplicateNames(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, ".saker", "commands")
