...
// TestNewOpenAI_ExtraBodyPlumbing locks in the wiring that turns each
// OpenAIConfig.ExtraBody entry into an option.WithJSONSet on every request,
// and proves the model defensively copies the map (so callers mutating their
// config later don't race with in-flight requests). Both invariants are
// non-obvious and easy to regress when the request path is refactored.
func TestNewOpenAI_ExtraBodyPlumbing(t *testing.T) {
	t.Parallel()
	cfg := OpenAIConfig{
		APIKey: "sk-test",
	...
	mdl, err := NewOpenAI(cfg)
	require.NoError(t, err)
	om, ok := mdl.(*openaiModel)
	require.True(t, ok, "expected *openaiModel")

	require.Len(t, om.extraBody, 2)
	require.Equal(t, true, om.extraBody["enable_thinking"])
	require.Len(t, om.extraBodyOpts(), 2,
		"each ExtraBody entry must turn into one WithJSONSet option")

	// Mutate caller's map: model state must not change.
	cfg.ExtraBody["enable_thinking"] = false
	delete(cfg.ExtraBody, "vendor_extra")
	require.Equal(t, true, om.extraBody["enable_thinking"],
		"model must hold a defensive copy of ExtraBody")
	require.Len(t, om.extraBody, 2)
}

// TestNewOpenAI_NoExtraBodyZeroAlloc keeps the vanilla OpenAI happy path
// allocation-free: when ExtraBody is nil/empty, extraBodyOpts must return nil
// so we don't push an empty slice through the SDK.
func TestNewOpenAI_NoExtraBodyZeroAlloc(t *testing.T) {
	t.Parallel()
	mdl, err := NewOpenAI(OpenAIConfig{APIKey: "sk-test"})
	require.NoError(t, err)
	om := mdl.(*openaiModel)
	require.Nil(t, om.extraBody)
	require.Nil(t, om.extraBodyOpts())
}
