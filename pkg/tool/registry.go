...
func connectMCPClientWithOptions(ctx context.Context, spec string, opts MCPServerOptions, handler mcpListChangedHandler) (*mcp.ClientSession, error) {
	...
	client := mcp.NewClient(&mcp.Implementation{Name: "saker", Version: "dev"}, clientOpts)
	...
	// NOTE: do not cancel dialCtx here — the SSE session uses it for its
	// ongoing connection.  The goroutine leak is already prevented by
	// close(done).  dialCtx will be GC'd when the session is closed.
	return session, nil
}
