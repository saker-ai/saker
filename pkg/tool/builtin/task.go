...
var taskSchema = &tool.JSONSchema{
	...
	Properties: map[string]interface{}{
		...
		"run_in_background": map[string]interface{}{
			"type":        "boolean",
			"description": "Set to true to run this agent in the background. Returns immediately with a task ID instead of waiting for completion.",
		},
		"isolation": map[string]interface{}{
			"type":        "string",
			"enum":        []string{"worktree"},
			"description": "Run in an isolated git worktree. The agent gets its own working copy of the repository.",
		},
	},
...
type TaskRequest struct {
	...
	Resume       string
	Background   bool
	Isolation    string // "worktree" or empty
}
...
type TaskTool struct {
	mu        sync.RWMutex
	runner    TaskRunner
	extraDesc string // dynamically appended agent descriptions
}
...
func (t *TaskTool) Description() string {
	...
	t.mu.RUnlock()
	if extra != "" {
		return taskToolDescription + extra
	}
	return taskToolDescription
}

// AppendAgentDescriptions adds dynamically detected agent type descriptions
// (e.g. ACP agents found on PATH) to the tool description so the model
// knows they are available as subagent_type values.
func (t *TaskTool) AppendAgentDescriptions(desc string) {
	t.mu.Lock()
	t.extraDesc = desc
	t.mu.Unlock()
}
...
func parseTaskParams(params map[string]interface{}) (TaskRequest, error) {
	...
	subagentType, _ := optionalString(params, "subagent_type")
	...
	isolation, err := optionalString(params, "isolation")
	if err != nil {
		return TaskRequest{}, err
	}
	isolation = strings.ToLower(strings.TrimSpace(isolation))
	if isolation != "" && isolation != "worktree" {
		return TaskRequest{}, fmt.Errorf("isolation must be \"worktree\", got %q", isolation)
	}
...
func optionalBool(params map[string]interface{}, key string) bool {
	raw, ok := params[key]
	if !ok {
		return false
	}
