...
func (g *GrepTool) searchDirectory(ctx context.Context, root string, re *regexp.Regexp, opts grepSearchOptions, matches *[]GrepMatch) (bool, error) {
	...
	err := filepath.WalkDir(root, func(path string, d fs.DirEntry, walkErr error) error {
		...
		if d.IsDir() {
			// Auto-load nested .gitignore files (like rg does).
			if opts.gitignoreMatcher != nil && path != root {
				relDir := displayPath(path, root)
				gitignorePath := filepath.Join(path, ".gitignore")
				if _, statErr := os.Stat(gitignorePath); statErr == nil {
					_ = opts.gitignoreMatcher.LoadNestedGitignore(relDir)
				}
			}
...
func (opts grepSearchOptions) allow(path string) (bool, error) {
	...
	var globOk bool
	if hasGlob {
		ok, err := matchGlobPattern(opts.glob, rel)
		if err != nil {
	...
	// When both glob and type are specified, glob takes precedence
	// (consistent with rg where --glob overrides --type).
	if hasGlob {
		return globOk, nil
	}
	return typeOk, nil
...
// matchGlobPattern matches a glob pattern against a relative path,
// supporting ** doublestar patterns (e.g., **/*.go, src/**).
func matchGlobPattern(pattern, name string) (bool, error) {
	if strings.Contains(pattern, "**") {
		return matchDoublestarGlob(pattern, name), nil
	}
	return filepath.Match(pattern, name)
}

// matchDoublestarGlob handles glob patterns containing **.
func matchDoublestarGlob(pattern, name string) bool {
	...
	// **/ at start — match suffix against any path suffix.
	if prefix == "" {
		if suffix == "" {
			return true
		}
		...
		// Also match just basename.
		if ok, _ := filepath.Match(suffix, filepath.Base(name)); ok {
			return true
		}
		return false
	}
	...
	// prefix/**/suffix — prefix must match, then suffix against remainder.
	if !strings.HasPrefix(name, prefix+"/") {
		return false
	}
	remainder := strings.TrimPrefix(name, prefix+"/")
	// Try suffix against remainder and its sub-paths.
	remainParts := strings.Split(remainder, "/")
	for i := 0; i < len(remainParts); i++ {
		sub := strings.Join(remainParts[i:], "/")
		if ok, _ := filepath.Match(suffix, sub); ok {
			return true
		}
	}
