...
func (g *GrepTool) Execute(ctx context.Context, params map[string]interface{}) (*tool.ToolResult, error) {
	...
	// Prefer ripgrep binary when available for better performance.
	if rgAvailable() {
		rgOpts := rgSearchOptions{
		...
		if rgMatches, rgTruncated, rgErr := rgSearch(ctx, pattern, targetPath, g.root, rgOpts); rgErr == nil {
			formatted := formatGrepOutput(outputMode, rgMatches, showLineNumbers, headLimit, offset, rgTruncated)
			data := map[string]interface{}{
				"pattern":          pattern,
		...
		// rg failed — fall through to pure-Go implementation.
	}
