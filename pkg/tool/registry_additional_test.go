...
func withStubMCPClient(t *testing.T, fn func(context.Context, string, mcpListChangedHandler) (*mcp.ClientSession, error)) func() {
	...
	originalWithOpts := newMCPClientWithOptions
	newMCPClient = fn
	newMCPClientWithOptions = func(ctx context.Context, spec string, _ MCPServerOptions, handler mcpListChangedHandler) (*mcp.ClientSession, error) {
		return fn(ctx, spec, handler)
	}
	return func() {
		newMCPClient = original
		newMCPClientWithOptions = originalWithOpts
	}
}
