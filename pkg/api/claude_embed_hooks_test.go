...
func TestMaterializeEmbeddedClaudeHooks(t *testing.T) {
	...
	root := t.TempDir()
	fs := fstest.MapFS{
		".saker/hooks/pre.sh":      {Data: []byte("echo pre")},
		".saker/hooks/sub/post.sh": {Data: []byte("echo post")},
	}
	if err := materializeEmbeddedClaudeHooks(root, fs); err != nil {
		t.Fatalf("materialize failed: %v", err)
	}
	data, err := os.ReadFile(filepath.Join(root, ".saker/hooks/pre.sh"))
	if err != nil || !strings.Contains(string(data), "pre") {
		t.Fatalf("unexpected hook content %q err=%v", data, err)
	}

	// existing file should not be overwritten
	dest := filepath.Join(root, ".saker/hooks/pre.sh")
...
func TestMaterializeEmbeddedClaudeHooksErrors(t *testing.T) {
	...
	readFS := readErrFS{FS: fstest.MapFS{".saker/hooks/pre.sh": {Data: []byte("hi")}}, err: errors.New("read failed")}
