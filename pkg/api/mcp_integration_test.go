...
func TestRegisterMCPServersNotBlockedByBuiltinWhitelist(t *testing.T) {
	...
	patchedNewMCPClientWithOptions = func(ctx context.Context, spec string, _ tool.MCPServerOptions, handler func(context.Context, *mcp.ClientSession)) (*mcp.ClientSession, error) {
		return counter.dial(ctx, spec, handler)
	}
	defer func() {
		patchedNewMCPClient = orig
