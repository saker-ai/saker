...
func TestTaskRunnerUnknownSubagentError(t *testing.T) {
	// When only general-purpose is registered, requesting an unknown subagent
	// type should fallback to general-purpose instead of returning an error.
	root := newClaudeProject(t)
	mdl := &stubModel{}
	...
	rt, err := New(context.Background(), Options{ProjectRoot: root, Model: mdl, Subagents: []SubagentRegistration{{
		Definition: def,
		Handler: subagents.HandlerFunc(func(context.Context, subagents.Context, subagents.Request) (subagents.Result, error) {
			return subagents.Result{Output: "fallback-ok"}, nil
		}),
	}}})
	if err != nil {
		t.Fatalf("runtime: %v", err)
	...
	runner := rt.taskRunner()
	result, err := runner(context.Background(), toolbuiltin.TaskRequest{
		Description:  "Plan routing fallback",
		Prompt:       "plan now",
		SubagentType: subagents.TypePlan,
	})
	if err != nil {
		t.Fatalf("expected fallback to general-purpose, got error: %v", err)
	}
