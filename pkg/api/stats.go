...
func (t *tokenTracker) Record(stats TokenStats) {
	...
	session.EstimatedCostUSD += stats.EstimatedCostUSD
	if stats.Currency != "" {
		session.Currency = stats.Currency
	}
	...
	if stats.Model != "" {
		...
		modelStats.EstimatedCostUSD += stats.EstimatedCostUSD
	...
	t.total.EstimatedCostUSD += stats.EstimatedCostUSD
	if stats.Currency != "" {
		t.total.Currency = stats.Currency
	}
...
// tokenStatsFromUsage converts model.Usage to TokenStats, including cost estimation.
func tokenStatsFromUsage(usage model.Usage, modelName, sessionID, requestID string) TokenStats {
	...
	if modelName != "" {
		cost := model.EstimateCost(modelName, usage)
		stats.EstimatedCostUSD = cost.TotalCost
