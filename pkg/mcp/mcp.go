...
type (
	...
	EmbeddedResource            = mcpsdk.EmbeddedResource
	ResourceContents            = mcpsdk.ResourceContents
...
func buildStdioTransport(ctx context.Context, cmdSpec string) (Transport, error) {
	...
	// Check for known malware before launching the process.
	if err := CheckPackageForMalware(parts[0], parts[1:]); err != nil {
		return nil, err
	}
