...
func TestFS_ReadFile_OSOnly(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".saker", "settings.json")
...
func TestFS_Open_ErrorFallback(t *testing.T) {
	...
	path := filepath.Join(dir, ".saker", "missing.txt")
...
func TestFS_ReadDir_EmbedOnly(t *testing.T) {
	dir := t.TempDir()
	fsys := NewFS(dir, buildEmbedFS())
	path := filepath.Join(dir, ".saker", "skills")
...
func TestFS_ReadDir_FallbackError(t *testing.T) {
	dir := t.TempDir()
	fsys := NewFS(dir, fstest.MapFS{})
	path := filepath.Join(dir, ".saker", "missing")
...
func TestFS_WalkDir_EmbedOnly(t *testing.T) {
	dir := t.TempDir()
	fsys := NewFS(dir, buildEmbedFS())
	root := filepath.Join(dir, ".saker", "walk")
	visited := map[string]struct{}{}
	if err := fsys.WalkDir(root, func(path string, d fs.DirEntry, walkErr error) error {
		if walkErr != nil {
...
func TestFS_WalkDir_FallbackError(t *testing.T) {
	dir := t.TempDir()
	fsys := NewFS(dir, fstest.MapFS{})
	root := filepath.Join(dir, ".saker", "missing")
...
func TestFS_WalkDir_EmbedRootFallback(t *testing.T) {
	...
	if err := fsys.WalkDir(dir, func(path string, d fs.DirEntry, walkErr error) error {
		...
		if strings.Contains(filepath.ToSlash(path), ".saker") {
			visited = true
		}
		return nil
	}); err != nil {
...
func TestFS_toEmbedPath(t *testing.T) {
	...
	cwdRelative := strings.TrimLeft(filepath.ToSlash(filepath.Join(cwd, ".saker", "file.json")), "/")
	...
	tests := []struct {
	...
	}{
		{
			...
			fs:   defaultFS,
			path: filepath.Join(dir, ".saker", "settings.json"),
			want: ".saker/settings.json",
		},
		{
			name: "relative path anchored to root",
			fs:   defaultFS,
			path: filepath.Join(".saker", "settings.local.json"),
			want: ".saker/settings.local.json",
		},
		{
			name: "windows separators normalized",
			fs:   windowsFS,
			path: `C:\project\.saker\settings.json`,
			want: ".saker/settings.json",
		},
		{
			name: ".saker subpath retained",
			fs:   defaultFS,
			path: filepath.Join(dir, ".saker", "skills", "demo"),
			want: ".saker/skills/demo",
		},
		{
		...
		},
		{
			...
			fs:   noRootFS,
			path: filepath.Join(".saker", "file.json"),
			want: cwdRelative,
		},
		{
			name: "windows UNC path",
			fs:   uncFS,
			path: `\\server\share\.saker\settings.json`,
			want: ".saker/settings.json",
		},
	}
...
func TestFS_FileNotFound(t *testing.T) {
	dir := t.TempDir()
	fsys := NewFS(dir, buildEmbedFS())
	path := filepath.Join(dir, ".saker", "missing.json")
...
func buildEmbedFS() fstest.MapFS {
	return fstest.MapFS{
		".saker":                       &fstest.MapFile{Mode: fs.ModeDir},
		".saker/settings.json":         &fstest.MapFile{Data: []byte(`{"source":"embed"}`)},
		".saker/skills":                &fstest.MapFile{Mode: fs.ModeDir},
		".saker/skills/alpha":          &fstest.MapFile{Mode: fs.ModeDir},
		".saker/skills/alpha/SKILL.md": &fstest.MapFile{Data: []byte("alpha")},
		".saker/skills/beta":           &fstest.MapFile{Mode: fs.ModeDir},
		".saker/skills/beta/SKILL.md":  &fstest.MapFile{Data: []byte("beta")},
		".saker/walk":                  &fstest.MapFile{Mode: fs.ModeDir},
		".saker/walk/inner":            &fstest.MapFile{Mode: fs.ModeDir},
		".saker/walk/inner/file.txt":   &fstest.MapFile{Data: []byte("walk")},
	}
}
