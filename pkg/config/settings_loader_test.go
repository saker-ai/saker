...
func loadSettings(t *testing.T, projectRoot string, runtimeOverrides *Settings) *Settings {
	...
	loader := SettingsLoader{
		...
		UserHome:         t.TempDir(), // isolate from real ~/.saker/settings.json
	}
...
func TestSettingsLoader_SingleLayer(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			loader := SettingsLoader{ProjectRoot: projectRoot, UserHome: t.TempDir()}
			got, err := loader.Load()
			require.NoError(t, err)
...
func TestSettingsLoader_UserGlobalLayer(t *testing.T) {
	t.Run("user-global provides base settings", func(t *testing.T) {
		t.Parallel()
		fakeHome := t.TempDir()

		globalPath := filepath.Join(fakeHome, ".saker", "settings.json")
		writeSettingsFile(t, globalPath, Settings{
		...
		loader := SettingsLoader{ProjectRoot: projectRoot, UserHome: fakeHome}
		got, err := loader.Load()
		require.NoError(t, err)
		require.Equal(t, "user-global-model", got.Model)
		require.Equal(t, "yes", got.Env["GLOBAL"])
	})

	t.Run("project overrides user-global", func(t *testing.T) {
		t.Parallel()
		fakeHome := t.TempDir()
		...
		writeSettingsFile(t, globalPath, Settings{
			Model: "user-global-model",
			Env:   map[string]string{"SHARED": "global", "ONLY_GLOBAL": "g"},
		})
		...
		writeSettingsFile(t, projectPath, Settings{
			Model: "project-model",
			Env:   map[string]string{"SHARED": "project"},
		})
		...
		require.Equal(t, "project", got.Env["SHARED"])
		require.Equal(t, "g", got.Env["ONLY_GLOBAL"]) // inherited from global
	})

	t.Run("full precedence: user-global < project < local < runtime", func(t *testing.T) {
		t.Parallel()
		fakeHome := t.TempDir()
		...
		writeSettingsFile(t, globalPath, Settings{
			Model: "global",
			Env:   map[string]string{"L": "global"},
		})
		...
		projectPath := getProjectSettingsPath(projectRoot, "")
		localPath := getLocalSettingsPath(projectRoot, "")
		writeSettingsFile(t, projectPath, Settings{Model: "project", Env: map[string]string{"L": "project"}})
		writeSettingsFile(t, localPath, Settings{Model: "local", Env: map[string]string{"L": "local"}})

		loader := SettingsLoader{ProjectRoot: projectRoot, UserHome: fakeHome, RuntimeOverrides: &Settings{Model: "runtime"}}
		got, err := loader.Load()
		require.NoError(t, err)
		require.Equal(t, "runtime", got.Model)
		require.Equal(t, "local", got.Env["L"])
	})
}
